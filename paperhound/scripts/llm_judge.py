...
# env pins outlines<1.0 (no from_openai), so call openrouter directly for json output.
SCHEMA_HINT = (
    "\nReturn ONLY a JSON object with keys: reasoning (string), "
    "faithfulness, answer_correctness, relevance, completeness (each integer 1-5)."
)
...
async def judge_one(sem: asyncio.Semaphore, idx: int, total: int, record: dict) -> dict:
    ...
    async with sem:
        ...
        try:
            score = JudgeScore.model_validate_json(content)
        except Exception:
            m = re.search(r"\{.*\}", content, re.DOTALL)
            score = JudgeScore.model_validate_json(m.group(0))
